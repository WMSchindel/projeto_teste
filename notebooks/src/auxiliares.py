...
def tabela_distribuicao_frequencias (dataframe, coluna, coluna_frequencia=False):
    ...
    """Cria uma tabela de distribuição de frequencias para uma coluna de um dataframe.
...
def composicao_histograma_boxplot(dataframe, coluna, intervalos="auto"):
    ...
    """Cria uma figura 2x1 contendo o gráfico de distribuição de frequencias e o boxplot.
    Espera um dataframe e uma coluna.
    ...
    dataframe : pd.DataFrame
        Dataframe com os dados.
    coluna : coluna
        Nome da coluna a ser classificada.
    intervalo = "auto" :
        Informa qual o método será utilizado para escolher o tamanho dos bins. Se permanecer "auto" escolhemos o método "sturges".
        Se for utilizado outro método informar ao invés de "auto".

    Returns
    -------
  
  Gráfico de distribuição de frequências e boxplot.
    """
...
def analise_shapiro(dataframe, alfa=0.05):
    ...
    for coluna in dataframe.columns:
        ...
        if valor_p_sw > alfa:
            print(f"{coluna} segue uma distribuição normal(valor_p:{valor_p_sw:.3f})")
        else:
            print(f"{coluna} não segue uma distribuição normal(valor_p:{valor_p_sw:.3f})")
...
def analise_levene(dataframe, alfa=0.05, centro="mean"):
    ...
    if valor_p_levene > alfa:
        print(f"Variâncias iguais (valor_p:{valor_p_levene:.3f})")
    else:
        print(f"Ao menos uma variância é diferente (valor_p:{valor_p_levene:.3f})")
...
def analise_ttest_ind(
    ...
    *[dataframe[coluna] for coluna in dataframe.columns], 
    ...
    if valor_p_ttest > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_ttest:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_ttest:.3f})") 
...
def analise_anova_one_way(
    ...
    *[dataframe[coluna] for coluna in dataframe.columns], 
    ...
    if valor_p_f > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_f:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_f:.3f})") 
...
def analise_wilcoxon(
    ...
    if valor_p_wilcoxon > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_wilcoxon:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_wilcoxon:.3f})")
...
def analise_mannwhitneyu(
    ...
    if valor_p_mw > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_mw:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_mw:.3f})")
...
def analise_friedman(
    ...
    if valor_p_friedman > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_friedman:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_friedman:.3f})")
...
def analise_kruskal(
    ...
    if valor_p_kruskal > alfa:
        print(f"Não rejeita a hipótese nula (valor_p:{valor_p_kruskal:.3f})")
    else:
        print(f"Rejeita a hipótese nula (valor_p:{valor_p_kruskal:.3f})")